...
class SAC(object):
    ...
    def dual_buff_update(
        self, memory, exp_memory, batch_size, updates, logger=None, reverse_mask=False
    ):
        # Sample a batch from memory
        ...
        (
            ...
            reward_batch,
            mask_batch,
        ) = memory.sample(half_batch_size).astuple()
        (
            exp_state_batch,
            exp_action_batch,
            exp_next_state_batch,
            exp_reward_batch,
            exp_mask_batch,
        ...
        next_state_batch = np.concatenate(
            (next_state_batch, exp_next_state_batch), axis=0
        )
